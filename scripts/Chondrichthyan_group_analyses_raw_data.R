...
#################
# Stacked area chart
# bind elasmo and holo data for easier plotting
...
areaplot <- ggplot(total) +
  ...
  scale_y_continuous(expand=c(0,0), breaks = c(0,20,40,60,80), limits = c(0, 80)) +
  theme_classic()+
  theme(legend.position=c(0.12, 0.9),legend.background = element_rect(size=0.5))+
  theme(plot.margin=margin(0.5,0.75,0.5,0.5,"cm"))


areaplot <-areaplot+ coord_geo(xlim = c(467.3, 251.902),  pos = as.list(rep("bottom",2)),
